...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 27 21:43:13 2023
...
#bkwd_time_1 will be from 30 to 0 to dt=-0.1
...
if __name__ == "__main__":
    ...
    # tried 0 to -30 with dt=-0.1 and didn't work
    t0 = 20         # initial time
    t1 = -20        # final time
    ...
    for frame in np.arange(0,interval+1,1):
        #frame = 0
        fig = plt.figure()
        ...
        sc = ax.scatter(gyre.states[:,frame,0], gyre.states[:,frame,1], 
                   s=15, c=lavd_array[:,frame])
        ...
        ax.set_title('LAVD at $A$='+str(A)+', $\epsilon$='+str(epsilon)+', $\omega$=$\pi$/5, $t$='+str(round(t0+frame*dt,2))+' to '+str(round(frame*dt+t0+interval*dt,2))+', $dt$='+str(dt))
        
        
        if saveVars == 'yes':
            save_y0_file = './saveSparse/fwd_time_1000/fields_A'+str(A)+'_eps'+str(epsilon)+'_omega'+str(float(round(omega,2)))+'_frame'+str(frame)+'.mat'
            sio.savemat(save_y0_file, {'gyre_x':gyre.states[:,frame,0], 'gyre_y':gyre.states[:,frame,1], 'lavd_field':lavd_array[:,frame]})
