...
def main(x0, xf, y0, yf, t0, t1, dt, numRows, numCols):
    ...
    # epsilon in np.arange(0.1, 1.2, 0.5):
    
    for omega in np.arange(3*np.pi/5, 2*np.pi, 2*np.pi/5):
        ...
        #if saveVars == 'yes':
        #    save_y0_file = './saveExplore/change_eps/A'+str(A)+'_eps'+str(epsilon)+'_omega'+str(float(round(omega,2)))+'_t'+str(t1)+'.mat'
        ...
        if saveVars == 'yes':
            save_y0_file = './saveExplore/change_eps/fields_A'+str(A)+'_eps'+str(epsilon)+'_omega'+str(float(round(omega,2)))+'_t'+str(t1)+'.mat'
