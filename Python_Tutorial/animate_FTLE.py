...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 12 00:07:43 2023
...
def main(x0, xf, y0, yf, t0, t1, dt, numRows, numCols):
    ...
    minPoint, = ax.plot([],[],'o', markerfacecolor='red', markersize=8)
    maxPoint, = ax.plot([],[],'o', markerfacecolor='cyan', markeredgecolor='hotpink', markersize=8)
    ...
    h0 = ax.pcolormesh(mesh[0], mesh[1], ftle_field)
    ...
    def update(frame):
        ...
        print("Computing FTLE...")
        ftle_field = geom.computeFTLE(J_array, t1-t0)
        title.set_text(u"FTLE at $A$={}, $\epsilon$={}, $\omega$={}, Time: {}".format(A, epsilon, float(round(omega,3)), float(round(dt*frame,2))) )
        ax.axis('scaled')
        ax.set_xlim([0,2])
        ax.set_ylim([0,1])
        i = np.unravel_index(ftle_field.argmin(), ftle_field.shape)
    ...
    ani_name = 'FTLE_A'+str(A)+'_epsilon'+str(epsilon)+'_omega'+str(float(round(omega,2)))+'_t'+str(t1)+'.mp4'
